$(document).ready(function () {
    ...
    function ShowAllCategorys() {
        $("table tbody").html("");
        $.ajax({
            url: "https://localhost:7012/api/Orders/GetAllHistoryOrderPending",
            type: "get",
                ...
                $.each(result, function (index, value) {
                    ...
                    if (value.status !== "approval") {
                        appendElement.append($("<td>").html("<button class=\"btn btn-warning updateBtn\" data-product-id=\"" + value.orderId + "\">View Detail</button>"
                            + "<button class=\"btn ml-2 btn-success approvalBtn\" data-product-id=\"" + value.orderId + "\">Approval</button>"
                            + "<button class=\"btn ml-2 btn-danger deleteBtn\" data-product-id=\"" + value.orderId + "\"><i class=\"fas fa-trash\" style=\"color: #ffffff;\"></i></button>"));
                    } else {
                        appendElement.append($("<td>").html("<button class=\"btn btn-warning updateBtn\" data-product-id=\"" + value.orderId + "\">View Detail</button>"));
                    }
    ...
    function fetchProductDetails(productId) {
        $.ajax({
            ...
            success: function (result, status, xhr) {
                ...
                $("#orderid_ap").text("");
                $("#orderId-apGet").val("");
                $("#orderid_ap").text("OrderID: " + product.orderId);
                $("#orderId-apGet").val(product.orderId);
            },
            error: function (xhr, status, error) {
                console.log("loi");
                console.log(xhr);
    ...
    $("#submitBtn-ap").click(function () {
        var productId = $("#orderId-apGet").val();
        //console.log(productId +"-----");

        // Send the form data to the API
        $.ajax({
            url: "https://localhost:7012/api/Orders/ApprovalRequestOrder/" + productId,
            type: "put",
            contentType: "application/json; charset=utf-8",
            headers: {
                "Authorization": "Bearer " + token
            ...
            success: function (result, status, xhr) {
                ...
                $('#productTable').DataTable().destroy();
                ShowAllCategorys();
                showSuccessToast("Successfully Approval");
            },
            error: function (xhr, status, error) {
                // Handle the error response
                console.log("loi");
